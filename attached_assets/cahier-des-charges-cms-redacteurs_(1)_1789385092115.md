...
**Statut :** MVP fonctionnel de bout en bout, vérifié en conditions réelles (rédaction → validation → traduction → publication → affichage app). Point différé consciemment noté en section 12, point 8 : pas de relecture des traductions avant publication (à ajouter en V2 si besoin).
...
- **Coût :** Supabase Edge Functions gratuites jusqu'à un quota mensuel généreux, GitHub Pages gratuit. **DeepL confirmé : crédit unique et définitif de 1 million de caractères, non renouvelable** (pas un quota mensuel) — une fois épuisé, la traduction automatique s'arrête définitivement sur ce compte, pas temporairement. Voir 8bis pour le comportement de repli.
...
6. ~~Conditions réelles de l'offre gratuite DeepL~~ — **Résolu, DeepL abandonné.** Le palier gratuit DeepL API s'est révélé être un crédit unique de 1M caractères non renouvelable (changement de politique DeepL, pas un quota mensuel comme supposé initialement), incompatible avec la contrainte "qui se renouvelle régulièrement". Remplacé par **MyMemory Translation API** (gratuit, sans carte, 5 000 caractères/jour anonyme ou 50 000/jour avec un email en paramètre — quota confirmé fonctionnel en conditions réelles, voir 8bis).
...
8. **Différé, non bloquant : pas d'écran de relecture des traductions avant publication.** L'approbation actuelle traduit et insère dans `dishes` en un seul geste — l'admin ne voit jamais le résultat de la traduction avant qu'il soit en production. Défaut concret observé sur une recette réelle : "th.7" (abréviation française de "thermostat 7") non traduit dans aucune des 4 langues cibles, laissé tel quel. Décision consciente de continuer sans cette étape pour l'instant ; à ajouter en V2 si la fréquence de ce genre d'incohérence devient gênante.
